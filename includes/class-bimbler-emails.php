<?php
/**
 * Bimbler Email Controller
 *
 * Bimbler Emails Class which handles the sending on transactional emails and email templates. This class loads in available emails.
 * Borrows heavily from the WooCommerce WC_Emails class.
 *
 * @class 		BImbler_Emails
 * @version		1.0.0
 * @package		Bimbler/Classes/Emails
 * @category	Class
 * @author 		Paul Perkins
 */
class Bimbler_Emails {
	...
	/**
	 * Bimbler RSVP Instance
	 *
	 * Ensures only one instance of Bimbler RSVP is loaded or can be loaded.
	 *
	 * @since 0.1
	 * @static
	 * @see WC()
	 * @return Main Bimbler RSVP instance
	 */
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, 'Cheatin&#8217; huh?');
	}
...
//		add_action( 'bimbler_email_header', array( $this, 'email_header' ) );
//		add_action( 'bimbler_email_footer', array( $this, 'email_footer' ) );
//		add_action( 'bimbler_email_order_meta', array( $this, 'order_meta' ), 10, 3 );

		// Hooks for sending emails during events
//		add_action( 'bimbler_low_stock_notification', array( $this, 'low_stock' ) );
//		add_action( 'bimbler_no_stock_notification', array( $this, 'no_stock' ) );
//		add_action( 'bimbler_product_on_backorder_notification', array( $this, 'backorder' ) );
//		add_action( 'bimbler_created_customer_notification', array( $this, 'customer_new_account' ), 10, 3 );

		// Let 3rd parties unhook the above via this hook
//		do_action( 'bimbler_email', $this );
	}
	...
	function init() {
		...
		include_once( 'abstracts/abstract-bimbler-email.php' );

		$this->emails['Bimbler_Email_New_Order']                 = include( 'emails/class-bimbler-email-new-order.php' );
		$this->emails['Bimbler_Email_Customer_Processing_Order'] = include( 'emails/class-bimbler-email-customer-processing-order.php' );
		$this->emails['Bimbler_Email_Customer_Completed_Order']  = include( 'emails/class-bimbler-email-customer-completed-order.php' );
		$this->emails['Bimbler_Email_Customer_Invoice']          = include( 'emails/class-bimbler-email-customer-invoice.php' );
		$this->emails['Bimbler_Email_Customer_Note']             = include( 'emails/class-bimbler-email-customer-note.php' );
		$this->emails['Bimbler_Email_Customer_Reset_Password']   = include( 'emails/class-bimbler-email-customer-reset-password.php' );
		$this->emails['Bimbler_Email_Customer_New_Account']      = include( 'emails/class-bimbler-email-customer-new-account.php' );
	...
	function get_from_name() {
		if ( ! $this->_from_name )
			$this->_from_name = get_option( 'bimbler_email_from_name' );
	...
	function get_from_address() {
		if ( ! $this->_from_address )
			$this->_from_address = get_option( 'bimbler_email_from_address' );
	...
	/**
	 * Wraps a message in the Bimbler mail template.
	 *
	 * @access public
	...
	function wrap_message( $email_heading, $message, $plain_text = false ) {
		...
		do_action( 'bimbler_email_header', $email_heading );

		echo wpautop( wptexturize( $message ) );

		do_action( 'bimbler_email_footer' );
