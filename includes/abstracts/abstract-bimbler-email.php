 ...
 *
 * Bimbler Email Class which is extended by specific email template classes to add emails to WooCommerce
 *
 * @class 		WC_Email
 * @version		2.0.0
 ...
 */
abstract class Bimbler_Email extends Bimbler_Settings_API {
	...
	function __construct() {
		...
		// Save settings hook
		add_action( 'bimbler_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );

		// Default template base if not declared in child constructor
		if ( is_null( $this->template_base ) ) {
			$this->template_base = WC()->plugin_path() . '/templates/'; // TODO: Fix this.
		}
		...
		add_filter( 'bimbler_email_style_inline_tags', array( $this, 'style_inline_tags' ) );
		add_filter( 'bimbler_email_style_inline_h1_tag', array( $this, 'style_inline_h1_tag' ) );
		add_filter( 'bimbler_email_style_inline_h2_tag', array( $this, 'style_inline_h2_tag' ) );
		add_filter( 'bimbler_email_style_inline_h3_tag', array( $this, 'style_inline_h3_tag' ) );
		add_filter( 'bimbler_email_style_inline_a_tag', array( $this, 'style_inline_a_tag' ) );
		add_filter( 'bimbler_email_style_inline_img_tag', array( $this, 'style_inline_img_tag' ) );
	}
	...
	function get_subject() {
		return apply_filters( 'bimbler_email_subject_' . $this->id, $this->format_string( $this->subject ), $this->object );
	}
	...
	function get_heading() {
		return apply_filters( 'bimbler_email_heading_' . $this->id, $this->format_string( $this->heading ), $this->object );
	}
	...
	function get_recipient() {
		return apply_filters( 'bimbler_email_recipient_' . $this->id, $this->recipient, $this->object );
	}
	...
	function get_headers() {
		return apply_filters( 'bimbler_email_headers', "Content-Type: " . $this->get_content_type() . "\r\n", $this->id, $this->object );
	}
	...
	function get_attachments() {
		return apply_filters( 'bimbler_email_attachments', array(), $this->id, $this->object );
	}
	...
	function is_enabled() {
		...
		return apply_filters( 'bimbler_email_enabled_' . $this->id, $enabled, $this->object );
	}
	...
	function style_inline_h1_tag($styles) {
		$styles['color'] = get_option( 'bimbler_email_text_color' );
	...
	function get_style_inline_tags() {
		return apply_filters( 'bimbler_email_style_inline_tags', array() );
	}
	...
	function get_style_inline_for_tag($tag) {
		$styles = apply_filters( 'bimbler_email_style_inline_' . $tag . '_tag',  array() );
		$css = array();
	...
	function get_from_name() {
		return wp_specialchars_decode( esc_html( get_option( 'bimbler_email_from_name' ) ), ENT_QUOTES );
	}
	...
	function get_from_address() {
		return sanitize_email( get_option( 'bimbler_email_from_address' ) );
	}
    ...
    function init_form_fields() {
    	$this->form_fields = array(
			'enabled' => array( /* TODO: Localise the text */
    ...
    public function process_admin_options() {
		...
		if ( ! empty( $_POST['template_html_code'] ) && ! empty( $this->template_html ) ) {
			...
			$saved	= false;
			$file	= get_stylesheet_directory() . '/bimbler-rsvp/' . $this->template_html;
		...
		if ( ! empty( $_POST['template_plain_code'] ) && ! empty( $this->template_plain ) ) {
    		...
    		$saved	= false;
			$file	= get_stylesheet_directory() . '/bimbler-rsvp/' . $this->template_plain;
		...
		if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) {
			...
			if ( ! empty( $_GET['move_template'] ) && ( $template = esc_attr( basename( $_GET['move_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if (  wp_mkdir_p( dirname( get_stylesheet_directory() . '/bimbler-rsvp/' . $this->$template ) ) && ! file_exists( get_stylesheet_directory() . '/woocommerce/' . $this->$template ) ) {
						...
						// Copy template file
						copy( $template_file, get_stylesheet_directory() . '/bimbler-rsvp/' . $this->$template );
			...
			if ( ! empty( $_GET['delete_template'] ) && ( $template = esc_attr( basename( $_GET['delete_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if ( file_exists( get_stylesheet_directory() . '/bimbler-rsvp/' . $this->$template ) ) {
						unlink( get_stylesheet_directory() . '/bimbler-rsvp/' . $this->$template );
		...
		<?php if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) { ?>
			...
			<?php
				...
				foreach ( $templates as $template => $title ) :
					...
					$local_file		= get_stylesheet_directory() . '/bimbler-rsvp/' . $this->$template;
					$core_file		= $this->template_base . $this->$template;
					$template_file	= apply_filters( 'woocommerce_locate_core_template', $core_file, $this->$template, $this->template_base );
					?>
					<div class="template <?php echo $template; ?>">
						...
						<?php } elseif ( file_exists( $template_file ) ) { ?>
							...
							<p>
								...
								<?php if ( ( is_dir( get_stylesheet_directory() . '/bimbler-rsvp/emails/' ) && is_writable( get_stylesheet_directory() . '/woocommerce/emails/' ) ) || is_writable( get_stylesheet_directory() ) ) { ?>
